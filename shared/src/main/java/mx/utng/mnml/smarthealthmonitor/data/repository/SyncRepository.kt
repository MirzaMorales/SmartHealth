...
class SyncRepository(
     ...
     * y actualiza Room si hay datos nuevos (upsert).
     */
    suspend fun sincronizarDesdeNeon(limite: Int = 50) = withContext(Dispatchers.IO) {
        try {
            val response = NeonClient.api.executeQuery(
                ...
                request = NeonRequest(
                    query = """
                        SELECT id, bpm, estado, dispositivo, hora FROM lecturas_fc
            ...
            android.util.Log.e("SYNC", "Error al descargar desde Neon: ${e.message}")
            throw e
        }
    }
